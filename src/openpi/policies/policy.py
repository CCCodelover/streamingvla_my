...
class Policy(BasePolicy):
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None) -> dict:  # type: ignore[misc]
        ...
        outputs = {
            "state": inputs["state"],
            "actions": actions,
            "action_states": torch.zeros_like(actions),
        }
    ...
    @override
    def streaming_infer(self, obs: dict, *, noise: np.ndarray | None = None) -> Iterator[dict]:
        # Make a copy since transformations may modify the inputs in place.
        inputs = jax.tree.map(lambda x: x, obs)
        inputs = self._input_transform(inputs)
        if not self._is_pytorch_model:
        ...
        observation = _model.Observation.from_dict(inputs)
        action_generator = self._sample_actions(sample_rng_or_pytorch_device, observation, **sample_kwargs)       

        for action_token in action_generator:
            
            if isinstance(action_token, dict) and action_token.get("norm_exceeded"):
